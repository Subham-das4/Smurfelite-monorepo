...
- [ ] Add `Dispute` model: order ref, buyer, seller, status, reason, details JSON, createdAt
- [ ] Add `DisputeStatus` enum: `OPEN`, `UNDER_REVIEW`, `RESOLVED_BUYER`, `RESOLVED_SELLER`, `CLOSED`
- [ ] Add `SellerWallet` model: `userId`, `pendingBalance`, `availableBalance`, `frozenBalance`
- [ ] Add `WalletLedger` model: type, amount, orderId?, disputeId?, note, createdAt
